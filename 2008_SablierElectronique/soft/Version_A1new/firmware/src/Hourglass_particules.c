...
/** Descriptive File Name
  ...
  @Summary
    Le code utilis� a �t� inspir� du code :
 *  https://github.com/fbrnc/Arduino_Hourglass
 * 
 *  de Fabrizio Branca
 * 
 *  Le code n'a pas �t� copi�. Mais il m'a aid� dans la reflexion de mon code.

  @Description
    Ce fichier contient des fonctions vari�es pour la gestion des particules
 *  dans une matrice led 16x16. Il est possible d'avoir une tr�s bonne 
 *  interaction des particules avec ce code. Neanmoins, L'acceleration des
 *  particules n'est pas ger�e dans ce code. Il est possible d'avoir une 
...
/** 
  ...
  @Summary
    Fonction qui permet de remplir la matrice avec un nombre de particules determin�
...
void fillParticules(Matrix* Matrice, Matrice_num WhichMatrix, int Nparticules, Gravity Sens)
{
    ...
    if(pcount < Nparticules) //mise � jours de la particule dans la position XY : 0,0
    {
    ...
    while(pcount < Nparticules) //boucle tant que nous n'avons pas mis les particules demand�es.
    {   
        if (y > 0)//pr�miere partie de la matrice, remplissage selon Y
        {    
            y--;//nous commen�ons en bas de la matrice et d�crementent Y � chaque fois que nous avons finis une ligne
            ...
            if(!getXY(Matrice,WhichMatrix,x,y,Sens)) //si la position est vide
                {
                   setXY(Matrice,WhichMatrix,x,y,true,Sens); //mise � 1
                   pcount ++;  
                }
            
            for(count = (15 - y); count > 0; count--) //v�rification de toute la ligne
            {
                xdec--;
                ydec++;
        ...
        else //D�uxieme partie, remplissage selon X
        {
            x--;
            ...
            for(count = x; count > 0; count--) //v�rification de la ligne
            {
                xdec--;
                ydec++;
...
void clearMatrice(Matrix* Matrice, Matrice_num WhichMatrix)
{
    ...
    switch(WhichMatrix)
    {
        case Matrice_TOP :
            for(i = 0; i < 16; i++)
            {
                Matrice->Top[i] = 0xffff;//mise � 0 des leds
            }
            break;
        case Matrice_BOT :
            for(i = 0; i < 16; i++)
            {
                Matrice->Bot[i] = 0xffff;//mise � 0 des leds
            }
            break;
    }
}
...
/** 
  ...
  @Summary
    Fonction qui permet de transferer une particule d'une matrice � l'autre
    Ce transfert se fait toujours � l'interception entre les deux matrices

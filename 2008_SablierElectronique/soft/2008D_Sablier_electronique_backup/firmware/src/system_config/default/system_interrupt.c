...
// timer 1 : 3 ms / pour l'app + debounce
void __ISR(_TIMER_1_VECTOR, ipl3AUTO) IntHandlerDrvTmrInstance0(void)
{
    ...
    DoDebounce (&BoutonBas, SW_DOWNStateGet());
    DoDebounce (&BoutonHaut, SW_UPStateGet());
    DoDebounce (&HMConfig, SW_SELStateGet());
}

//timer 2 @ 2 kHz pour le scan des lignes

...
int GetMessage(StruMess *pData)
{
    ...
    if(NbCharToRead >= MESS_SIZE) //Si il y a un message dans le FIFO
    {
        ...
        if(RxMess.Start == 0xAA)// si il correspond a la valeur defini pour le premier caractere
        {
            ...
            if (((RxMess.Heure <= 4)&&(RxMess.Minute <= 60))||((RxMess.Heure == 0)&&(RxMess.Minute >= 5)))
            {
           
                pData->Start = RxMess.Start;
                pData->Heure = RxMess.Heure;
                pData->Minute = RxMess.Minute;
            ...
            else //Si la valeur est incorrect
            {
                commStatus = 2;//erreur des valeurs
            }
...
void SendMessage(StruMess *pData)
{
    ...
    TxMess.Heure = pData->Heure;     // Speed
    TxMess.Minute = pData->Minute;     // Angle
...
//    TxMess.LsbCrc = CRC16 & 0xFF;          
    ...
    if (GetWriteSpace(&descrFifoTX) >= 5)  
    {
            ...
            PutCharInFifo(&descrFifoTX, TxMess.Heure);
            PutCharInFifo(&descrFifoTX, TxMess.Minute);
...
void __ISR(_UART_1_VECTOR, ipl5AUTO) _IntHandlerDrvUsartInstance0(void)
{
    ...
    if ( PLIB_INT_SourceFlagGet(INT_ID_0, INT_SOURCE_USART_1_TRANSMIT) &&
        ...
        {
            if((!PLIB_USART_TransmitterBufferIsFull(USART_ID_1)) && (GetReadSize(&descrFifoTX))) 
            {

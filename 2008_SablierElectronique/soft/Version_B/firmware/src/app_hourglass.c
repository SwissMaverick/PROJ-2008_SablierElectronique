/*******************************************************************************
  ...
  Modifie par Carlos Antonio Reyes Peña
  Modifier par Mario Dos Santos 07.05.21
 *  
  File Name:
    app.c
...
#include "Invn/Drivers/Iam20680/Iam20680Driver_HL.h"
#include "Invn/Drivers/Iam20680/Iam20680Defs.h"
#include <math.h>
// *****************************************************************************
// *****************************************************************************
...
void APP_HOURGLASS_Initialize ( void )
{
...
void APP_HOURGLASS_Tasks ( void )
{
    /* Check the application's current state. */
    static ConfigHM EtatMatrice = Started;
    static uint16_t Minutes = 1;
    static uint16_t Heures = 0;
    static uint8_t CountClignote;
    static uint8_t ApresAlarm;
    switch ( appData.state )
    {
        /* Application's initial state. */
        case APP_HOURGLASS_STATE_INIT:
        {
            //effacement des matrices a leds
            ...
            StartSensM = ToptoBot;
            TimeCount = Minutes + (Heures*60); //TimeCount est la valeur utilise dans le timer
           ...
           /* DRV_TMR0_Initialize();
            ...
            appData.state = APP_HOURGLASS_STATE_SERVICE_TASKS;
            break;
        }
        case APP_HOURGLASS_STATE_WAIT : 
        {
              // nothing to do
            break; 
        }
        ...
        case APP_HOURGLASS_STATE_SERVICE_TASKS:
        {
            if(AlarmSound == 0)
            {
                ...
                if((abs(abs(IAMVals[1])) > (abs(IAMVals[2])/4) || (abs(IAMVals[0]) > abs(IAMVals[2])/4)))//si les axes X ou Y sont plus grands que la valeur
                //de l'axe z divise par 4
                {
                ...
                {
                    SensM = Stop;// si la valeur de l'axe z divise par 4 est plus grande que la valeur des axes x et y
                }
                if(DebounceIsReleased(&HMConfig)) // Le bouton est un normal ON
                {
                    DebounceClearReleased(&HMConfig);// nous effacons l'etat de l'appui
                ...
                switch(EtatMatrice)//selon l'état de la matrice
                {
                    ...
                    case ConfigM :
                        ...
                        {//bouton haut appuye, incremantation des minutes
                        ...
                        {//bouton bas appuye, decrementation des minutes
                        ...
                        if(CountClignote < 2)//eteint pendant 2/20 du temps allume
                        {
                        ...
                        else if (CountClignote < 20)//alume
                        {
                    ...
                    case ConfigH :
                        if(DebounceIsReleased(&BoutonHaut) ) // Le bouton est un normal ON
                        {//bouton haut appuye, incremantation des heures
                            DebounceClearReleased(&BoutonHaut);
                            if(Heures < 9)
                            {
                                Heures ++;
                        ...
                        if(DebounceIsReleased(&BoutonBas)) // Le bouton est un normal ON
                        {//bouton bas appuye, decrementation des heures
                            DebounceClearReleased(&BoutonBas);
                            if(Heures > 0)
                            {
                                Heures --;
            ...
            appData.state = APP_HOURGLASS_STATE_WAIT;
            break;
        }
...
void APP_HOURGLASS_UpdateState ( APP_HOURGLASS_STATES NewState )
{

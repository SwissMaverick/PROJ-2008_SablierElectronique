 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_smplrt_div( uint8_t new_value)
{
	return inv_iam20680_write_reg( MPUREG_SMPLRT_DIV, 1, &new_value);
}

/** @brief Set CONFIG register FIFO_COUNT_REC bit
 *
 *  Selects whether FIFO_COUNT[12:0] indicates the number of records or number of bytes. <br>
 *  When set to ‘1’ report FIFO count in terms of records. <br>
 *  When set to ‘0’ report FIFO count in terms of bytes.
 *
 *  @param[in] s Pointer to the driver context structure inv_iam20680
 *  @param[in] new_value See enum IAM20680_CONFIG_FIFO_COUNT_REC_t
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_config_fifo_count_rec( IAM20680_CONFIG_FIFO_COUNT_REC_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_CONFIG, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_CONFIG, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_config_fifo_mode( IAM20680_CONFIG_FIFO_MODE_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_config_dlpf_cfg( IAM20680_CONFIG_DLPF_CFG_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_gyro_config_fs_sel( IAM20680_GYRO_CONFIG_FS_SEL_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_GYRO_CONFIG, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_GYRO_CONFIG, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_gyro_config_fs_sel( IAM20680_GYRO_CONFIG_FS_SEL_t * value)
{
	int status;

	status = inv_iam20680_read_reg( MPUREG_GYRO_CONFIG, 1, (uint8_t *)value);
	if(status)
		return status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_gyro_config_fchoice_b( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_config_accel_fs_sel( IAM20680_ACCEL_CONFIG_FS_SEL_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_ACCEL_CONFIG, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_ACCEL_CONFIG, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_accel_config_accel_fs_sel( IAM20680_ACCEL_CONFIG_FS_SEL_t * value)
{
	int status;

	status = inv_iam20680_read_reg( MPUREG_ACCEL_CONFIG, 1, (uint8_t *)value);
	if(status)
		return status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_config2_dec2_cfg( IAM20680_ACCEL_CONFIG2_DEC2_CFG_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_ACCEL_CONFIG_2, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_ACCEL_CONFIG_2, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_config2_accel_fchoice_b( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_config2_a_dlpf_cfg( IAM20680_ACCEL_CONFIG2_A_DLPF_CFG_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_lp_mode_cfg_gyro_cycle( IAM20680_LP_MODE_CFG_GYRO_CYCLE_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_LP_MODE_CONFIG, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_LP_MODE_CONFIG, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_lp_mode_cfg_g_avgcfg( IAM20680_LP_MODE_CFG_G_AVGCFG_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_wom_thr( uint8_t new_value)
{
	return 	inv_iam20680_write_reg( MPUREG_ACCEL_WOM_THR, 1, &new_value);
}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_fifo_en_gyro_fifo_en( IAM20680_FIFO_EN_GYRO_FIFO_EN_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_FIFO_EN, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_FIFO_EN, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_fifo_en_accel_fifo_en( IAM20680_FIFO_EN_ACCEL_FIFO_EN_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_fifo_en_temp_fifo_en( IAM20680_FIFO_EN_TEMP_FIFO_EN_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_int_enable_wom_int_en( uint8_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_INT_ENABLE, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_INT_ENABLE, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_int_enable_data_rdy_int_en( IAM20680_INT_ENABLE_DATA_RDY_INT_EN_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_int_status( uint8_t* value)
{
	return inv_iam20680_read_reg( MPUREG_INT_STATUS, 1, value);
}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_intel_ctrl_accel_intel_en( IAM20680_ACCEL_INTEL_CTRL_ACCEL_INTEL_EN_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_ACCEL_INTEL_CTRL, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_ACCEL_INTEL_CTRL, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_intel_ctrl_accel_intel_mode( IAM20680_ACCEL_INTEL_CTRL_ACCEL_INTEL_MODE_t new_value)
{
	uint8_t value;
	int status;
...
/** @brief Set ACCEL_INTEL_CTRL register WOM_TH_MODE bit
 *
 *  0 – Set WoM interrupt on the OR of all enabled accelerometer thresholds <br>
 *  1 – Set WoM interrupt on the AND of all enabled accelerometer threshold <br>
 *  Default setting is 0
 *
 *  @param[in] s Pointer to the driver context structure inv_iam20680
 *  @param[in] new_value See enum IAM20680_ACCEL_INTEL_CTRL_WOM_TH_MODE_t
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_accel_intel_ctrl_wom_th_mode( IAM20680_ACCEL_INTEL_CTRL_WOM_TH_MODE_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_user_ctrl_fifo_en( IAM20680_USER_CTRL_FIFO_EN_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_USER_CTRL, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_USER_CTRL, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_user_ctrl_fifo_rst()
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_device_reset()
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_PWR_MGMT_1, 1, &value);
	if(status)
		return status;

	value |= BIT_DEVICE_RESET_MASK;

	status = inv_iam20680_write_reg( MPUREG_PWR_MGMT_1, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_sleep( IAM20680_PWR_MGMT_1_SLEEP_t new_value)
{
	uint8_t value;
	int status;
 ...
 *
 *  @param[in] s Pointer to the driver context structure inv_iam20680
 *  @param[in] new_value See enum IAM20680_PWR_MGMT_1_CYCLE_t
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_cycle( IAM20680_PWR_MGMT_1_CYCLE_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_gyro_stdby( IAM20680_PWR_MGMT_1_GYRO_STDBY_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_temp_dis( IAM20680_PWR_MGMT_1_TEMP_DIS_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_clksel( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_pwr_mgmt_1( uint8_t* value)
{
	return inv_iam20680_read_reg( MPUREG_PWR_MGMT_1, 1, value);
}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_2_accel_stby( uint8_t new_value)
{
	uint8_t value;
	int status;

	status = inv_iam20680_read_reg( MPUREG_PWR_MGMT_2, 1, &value);
	if(status)
		return status;
	...
	status = inv_iam20680_write_reg( MPUREG_PWR_MGMT_2, 1, &value);
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_2_fifo_lp_en( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_lp_mode_cfg_set_lp_accel_odr( double odr_hz)
{
	uint8_t value;
	int status;
	int i, idx = 10; /* default odr set to 250Hz */
	double avg_odr_hz;
	const IAM20680_LP_MODE_CONFIG_LPOSC_CLKSEL_t lp_mode_config_lposc_clksel_tbl[12] ={ {0, 0.24}, {1, 0.49}, {2, 0.98}, {3, 1.95},
																						 {4, 3.91}, {5, 7.81}, {6, 15.63},{7, 31.25},
																						 {8, 62.50},{9, 125.0},{10,250.0},{11,500.0}};
	int odr_tbl_len = sizeof(lp_mode_config_lposc_clksel_tbl)/sizeof(lp_mode_config_lposc_clksel_tbl[0]);

	for( i=0; i < odr_tbl_len-1; i++) {
		if( odr_hz <= lp_mode_config_lposc_clksel_tbl[i+1].odr_hz ){
			avg_odr_hz = (lp_mode_config_lposc_clksel_tbl[i].odr_hz + lp_mode_config_lposc_clksel_tbl[i+1].odr_hz)/2;
			idx = (odr_hz < avg_odr_hz) ? lp_mode_config_lposc_clksel_tbl[i].idx : lp_mode_config_lposc_clksel_tbl[i+1].idx;
			break;
		}
	}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_2_gyro_stby( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_wr_pwr_mgmt_1_temp_ctrl( uint8_t new_value)
{
	uint8_t value;
	int status;
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_fifo_count( uint8_t* value)
{
	return inv_iam20680_read_reg( MPUREG_FIFO_COUNTH, 2, value);
}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_fifo( int len, uint8_t* value)
{
	return inv_iam20680_read_reg( MPUREG_FIFO_R_W, len, value);
}
 ...
 *  @return 0 in case of succes negative value on error. See enum inv_status
 */
int inv_iam20680_rd_who_am_i(uint8_t * value)
{
	return inv_iam20680_read_reg(MPUREG_WHO_AM_I, 1, value);
}
